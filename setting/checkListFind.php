...
$whatfind = trim($_POST['whatfind']);
$whatfind = mb_eregi_replace("[^a-zа-яё0-9 ]", '', $whatfind);
$belongs = $_POST['belongs'];

if ((int)$belongs == 1) {
    $recipes = R::findAll('minirecipes', 'name LIKE ? AND authorid = ? AND ingestion = ?', ["%$whatfind%", $userid, "1"]);
...
elseif ((int)$belongs == 2) {
    $recipes = R::findAll('minirecipes', 'name LIKE ? AND authorid = ? AND ingestion = ?', ["%$whatfind%", $userid, "2"]);
    echo ('<div id="informModal">');
    foreach (array_reverse($recipes) as $recipeelem) {
        echo ('
                <hr class="recipeLine">
...
elseif ((int)$belongs == 34) {
    $recipes = R::findAll('minirecipes', 'name LIKE ? AND authorid = ? AND (ingestion = ? OR ingestion = ?)', ["%$whatfind%", $userid, "3", "4"]);
    echo ('<div id="informModal">');
    foreach (array_reverse($recipes) as $recipeelem) {
        echo ('
                <hr class="recipeLine">
